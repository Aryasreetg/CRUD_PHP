...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($regno_err) && empty($name_err) && empty($iemail_err) && empty($pemail_err) && empty($mobile_err)){
        ...
        $sql = "UPDATE students SET regno=?,name=?,iemail=?, pemail=?, mobile=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
           
            mysqli_stmt_bind_param($stmt, "sssssi", $param_regno, $param_name, $param_pemail, $param_iemail, $param_mobile, $param_id);
